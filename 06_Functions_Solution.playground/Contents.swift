...
// argument label has to be on call site and parameter has to be on scope
...
func getAreaOfCircle(radius: Double) -> Double // function signature
{
...
// Because there is no argument label at the call site, we need to use _ when we define our function(or don't write any labels) . We could have defined the function as increaseByOne(numberToIncrease: Int) but that's not what the example asks for, and it doesn't read as nicely.
